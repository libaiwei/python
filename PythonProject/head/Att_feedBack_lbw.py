...
datas = []
raw_datas = np.empty(shape=(250L,8L))
class getData:
...
class compute:
    ...
    def computer(dat, Att_base, opt):
        ...
        a = np.mat(dat)
        a_T = a.T[2250:,:8]
        raw_datas = np.append(raw_datas,a_T,axis=0)
        ...
        Att = np.mean(Att)
        Att = Att/Att_base
        print pw_alpha, pw_beta,Att

        #store data

        data = {}
        data['pw_alpha'] = pw_alpha
        data['pw_beta'] = pw_beta
        data['Att'] = Att
        datas.append(data)
        print data
        if len(datas)>300:
            scio.savemat('data/wcy/data_ear_head24.mat', {'matrix': raw_datas.T[:8,250:]})
            with open('data/wcy/data_ear_head24.json', 'w') as f:
                json.dump(datas,f)
            print 'done'
            exit()
