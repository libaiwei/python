...
def manager(opt):
    ...
    while True:
        ...
        dat_win = getDataFrame(tm, opt)
        flg_start = getStartFlag(tm, opt)       
        ...
        if flg_start == 1 and dat_win.shape >= opt['w_win']*opt['sr']:
            # calc Att
            Att = computer(dat_win, Att_base, opt)
            print "Att=%f" %Att
            if flag_has_base == 0:
...
def computer(dat, Att_base, opt):
    ...
    pw_alpha = calPower(dat_alpha, kk, opt)
    
    # beta band
    dat_beta = dat.copy()
    dat_beta.filter(16., 31., verbose='ERROR')
    pw_beta = calPower(dat_beta, kk, opt)
    
    # Att
    Att = pw_beta[0:1]/pw_alpha[0:1]
    Att = np.mean(Att)
    Att = Att/Att_base
    return Att
