...
class spider:
    ...
    def get_Movie(self,person):
        ...
        movies = []
        pattern_1 = re.compile('<span class="rating\d-t"></span>')
        # for name in names:
        ...
        for (name,info,f) in zip(names,infos,films):
            ...
            print 'insert into movie_table values("' + person + '","' + match.group(1) + '","' + name + '","' + info.get_text()  + '","' + score + '");'
    ...
    def start(self):
        people = ['gto14','applejx8888','146298388']
        for person in people:
            self.get_Movie(person)
            time.sleep(3)
spider = spider()
spider.start()